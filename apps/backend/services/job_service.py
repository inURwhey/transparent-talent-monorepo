...
class JobService:
    ...
    def _call_gemini_api(self, prompt, model_name="gemini-1.5-flash"):
        ...
        # CORRECTED: Updated to the stable v1 endpoint and modern model names
        # The 'generateContent' part is part of the method, not the model name.
        # The model name should be just 'gemini-1.5-flash' or 'gemini-1.5-pro-latest'.
        ...
        # The payload structure is also slightly different for the v1 API
        payload = {
    ...
    def _lightweight_scrape(self, url):
        try:
            ...
            return {
                ...
                "extracted_location": None # Location extraction logic can be added later
            }
        except requests.exceptions.RequestException as e:
    ...
    def analyze_job_posting(self, job_text, user_profile_data, company_profile_data=None):
        ...
        Strictly conform to the JSON structure. If a field cannot be determined, use null or an appropriate default.
        Your response MUST be valid JSON. Do not wrap it in markdown ```json ... ```.
        """
    ...
    def create_or_get_canonical_job(self, url: str, user_id: int, commit: bool = True):
        ...
        if not canonical_job:
            ...
            canonical_job = Job(
                ...
                job_title=ai_analysis_data.get('job_title', job_title), # Prefer AI title
                status='Active',
    ...
    def trigger_reanalysis_for_user(self, user_id: int):
        ...
        for job in jobs_to_reanalyze:
            ...
            job_description = job.notes # Assuming notes has the full description
            if job_description:
