...
import json
import time # Import time for rate limiting/pauses

# --- Constants (Duplicate from app.py, as this script is standalone for import) ---
ANALYSIS_PROTOCOL_VERSION = '2.0'
REQUEST_TIMEOUT = 15 # Timeout for external HTTP requests (job scraping)
AI_REQUEST_TIMEOUT = 120 # Timeout for AI API requests (can be longer)
SCRAPE_DELAY = 0.5 # Delay between scraping requests to be polite and avoid blocks
...
def run_job_analysis(job_description_text, user_profile_text):
    ...
    genai.configure(api_key=os.getenv('GEMINI_API_KEY')) # Re-configure to ensure it's loaded
    ...
    """
    # Use the AI_REQUEST_TIMEOUT for generative AI calls
...
def run_remediation_internal():
    ...
    try:
        ...
        conn.autocommit = False # Ensure transactions for atomicity
        with conn.cursor(cursor_factory=DictCursor) as cursor:
            ...
            for i, row in enumerate(jobs_to_reanalyze):
                ...
                try:
                    ...
                    conn.commit() # Commit each analysis individually for better progress visibility
                ...
                # Add a small delay between requests to avoid hitting rate limits or being blocked by job sites
        ...
        # If a DB error occurs early, mark all remaining as failed
        ...
        results['status'] = 'FAILED'
        # If an overall error occurs, mark all remaining as failed
        results['failed_count'] = results['total_to_process'] - results['processed_count']
    finally:
        if conn:
            conn.close()
