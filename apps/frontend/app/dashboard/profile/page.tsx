...
export default function UserProfilePage() {
    ...
    const handleSubmit = useCallback(async (e: FormEvent) => {
        ...
        try {
            // Create a payload without the fields that should not be sent
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Collapsible open={isPersonalInfoOpen} onOpenChange={setIsPersonalInfoOpen} className="border rounded-md shadow-sm bg-gray-50">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg text-gray-700 hover:bg-gray-100">
                            Contact & Basic Information {isPersonalInfoOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                        </CollapsibleTrigger>
                        <CollapsibleContent className="p-4 bg-white space-y-4">
                            ...
                            <div>
                                ...
                                {(profile.latitude && profile.longitude) && (
                                    <p className="text-xs text-gray-500 mt-1">Lat: {profile.latitude.toFixed(4)}, Lon: {profile.longitude.toFixed(4)}</p>
                                )}
                            </div>
                            <div>
                    ...
                    <Collapsible open={isCareerGoalsOpen} onOpenChange={setIsCareerGoalsOpen} className="border rounded-md shadow-sm bg-gray-50">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg text-gray-700 hover:bg-gray-100">
                            Career Aspirations {isCareerGoalsOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                        </CollapsibleTrigger>
                        <CollapsibleContent className="p-4 bg-white space-y-4">
                            {/* ... Fields ... */}
                        </CollapsibleContent>
                    ...
                    <Collapsible open={isWorkEnvOpen} onOpenChange={setIsWorkEnvOpen} className="border rounded-md shadow-sm bg-gray-50">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg text-gray-700 hover:bg-gray-100">
                            Work Environment & Requirements {isWorkEnvOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                        </CollapsibleTrigger>
                        <CollapsibleContent className="p-4 bg-white space-y-4">
                            {/* ... Fields ... */}
                        </CollapsibleContent>
                    ...
                    <Collapsible open={isSkillsOpen} onOpenChange={setIsSkillsOpen} className="border rounded-md shadow-sm bg-gray-50">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg text-gray-700 hover:bg-gray-100">
                            Skills & Industry Focus {isSkillsOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                        </CollapsibleTrigger>
                        <CollapsibleContent className="p-4 bg-white space-y-4">
                            {/* ... Fields ... */}
                        </CollapsibleContent>
                    ...
                    <Collapsible open={isPersonalityOpen} onOpenChange={setIsPersonalityOpen} className="border rounded-md shadow-sm bg-gray-50">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg text-gray-700 hover:bg-gray-100">
                           Personality & Self-Assessment {isPersonalityOpen ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                        </CollapsibleTrigger>
                        <CollapsibleContent className="p-4 bg-white space-y-4">
                           {/* ... Fields ... */}
                        </CollapsibleContent>
