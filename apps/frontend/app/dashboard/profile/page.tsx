...
import { ChevronDown, ChevronUp, MapPin, XCircle } from 'lucide-react'; // Added XCircle for clear button

interface Profile {
    id: number; user_id: number; full_name: string | null; current_location: string | null;
    latitude: number | null; longitude: number | null; linkedin_profile_url: string | null;
    resume_url: string | null; short_term_career_goal: string | null; long_term_career_goals: string | null;
    desired_annual_compensation: string | null; desired_title: string | null; ideal_role_description: string | null;
    preferred_company_size: string | null; ideal_work_culture: string | null; disliked_work_culture: string | null;
    core_strengths: string | null; skills_to_avoid: string | null; non_negotiable_requirements: string | null;
    deal_breakers: string | null; preferred_industries: string | null; industries_to_avoid: string | null;
    personality_adjectives: string | null; personality_16_personalities: string | null;
    personality_disc: string | null; personality_gallup_strengths: string | null;
...
export default function UserProfilePage() {
    ...
    const [openSections, setOpenSections] = useState({
        personalInfo: true, careerGoals: true, workEnv: true, skills: true, personality: true
    });
    ...
    const updateProfileData = useCallback(async (payload: Partial<Profile>) => {
        ...
        try {
            ...
            setProfile(updatedProfile); // Update state with the full returned profile
    ...
    const handleGetLocation = useCallback(() => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                updateProfileData({ latitude, longitude });
    ...
    // --- NEW: Handler to clear location data ---
    ...
    if (error && !profile) return <div className="min-h-screen flex items-center justify-center text-center">Error loading profile. Please refresh.</div>;
    ...
    return (
        <main className="min-h-screen bg-gray-50 p-8 font-sans">
            <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
                ...
                {successMessage && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert"><strong className="font-bold">Success!</strong><span className="block sm:inline"> {successMessage}</span></div>}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    {(profile.latitude && profile.longitude) && (
                                        <Button type="button" onClick={handleClearLocation} variant="ghost" size="icon" aria-label="Clear location"><XCircle className="h-4 w-4 text-red-500" /></Button>
                                    )}
                                </div>
                    ...
                    <Collapsible open={openSections.workEnv} onOpenChange={() => toggleSection('workEnv')} className="border rounded-md shadow-sm">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg">Work Environment & Requirements {openSections.workEnv ? <ChevronUp/> : <ChevronDown/>}</CollapsibleTrigger>
                        <CollapsibleContent className="p-4 pt-0 space-y-4">
                            <div>
                                <Label htmlFor="preferred_work_style">Preferred Work Style</Label>
                                <Select value={profile.preferred_work_style || 'null'} onValueChange={(value) => handleChange('preferred_work_style', value)}>
                            ...
                            {profile.preferred_work_style !== 'On-site' && (
                                <div className="flex items-center space-x-2"><Checkbox id="is_remote_preferred" checked={profile.is_remote_preferred || false} onCheckedChange={(checked: boolean) => handleCheckboxChange('is_remote_preferred', checked)} /><Label htmlFor="is_remote_preferred">I prefer remote work generally.</Label></div>
                            )}
                            <div>
                                <Label htmlFor="preferred_company_size">Preferred Company Size</Label>
                                <Select value={profile.preferred_company_size || 'null'} onValueChange={(value) => handleChange('preferred_company_size', value)}>
                                    <SelectTrigger><SelectValue placeholder="No Preference" /></SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="null">No Preference</SelectItem>
                    ...
                    <Collapsible open={openSections.skills} onOpenChange={() => toggleSection('skills')} className="border rounded-md shadow-sm">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg">Skills & Industry Focus {openSections.skills ? <ChevronUp/> : <ChevronDown/>}</CollapsibleTrigger>
                        <CollapsibleContent className="p-4 pt-0 space-y-4">
                            ...
                            <div><Label htmlFor="preferred_industries">Preferred Industries</Label><Textarea id="preferred_industries" value={profile.preferred_industries || ''} onChange={(e) => handleChange('preferred_industries', e.target.value)} rows={2} /></div>
                            <div><Label htmlFor="industries_to_avoid">Industries to Avoid</Label><Textarea id="industries_to_avoid" value={profile.industries_to_avoid || ''} onChange={(e) => handleChange('industries_to_avoid', e.target.value)} rows={2} /></div>
                        </CollapsibleContent>
                    </Collapsible>
                    
                    {/* Collapsible: Personality & Self-Assessment */}
                    <Collapsible open={openSections.personality} onOpenChange={() => toggleSection('personality')} className="border rounded-md shadow-sm">
                        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 font-semibold text-lg">Personality & Self-Assessment {openSections.personality ? <ChevronUp/> : <ChevronDown/>}</CollapsibleTrigger>
                        <CollapsibleContent className="p-4 pt-0 space-y-4">
                            <div><Label htmlFor="personality_adjectives">Personality Adjectives</Label><Textarea id="personality_adjectives" value={profile.personality_adjectives || ''} onChange={(e) => handleChange('personality_adjectives', e.target.value)} rows={2} /></div>
                            <div><Label htmlFor="personality_16_personalities">16 Personalities Type</Label><Input id="personality_16_personalities" type="text" value={profile.personality_16_personalities || ''} onChange={(e) => handleChange('personality_16_personalities', e.target.value)} placeholder="e.g., INTJ-T" /></div>
                            <div><Label htmlFor="personality_disc">DISC Profile</Label><Input id="personality_disc" type="text" value={profile.personality_disc || ''} onChange={(e) => handleChange('personality_disc', e.target.value)} placeholder="e.g., D-I" /></div>
                            <div><Label htmlFor="personality_gallup_strengths">Gallup Strengths</Label><Textarea id="personality_gallup_strengths" value={profile.personality_gallup_strengths || ''} onChange={(e) => handleChange('personality_gallup_strengths', e.target.value)} rows={2} /></div>
                        </CollapsibleContent>
                    </Collapsible>
