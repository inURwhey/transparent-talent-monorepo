# Transparent Talent: Product Backlog & Roadmap v1.15
...
### Session Cost
The "Session Cost" column estimates the AI Studio session token budget required to complete the task, which is the primary constraint on development velocity. This helps in planning development sessions.

*   **S (Small):** < 40k tokens. A task that can be completed quickly with minimal context, suitable for a short session.
*   **M (Medium):** 40k - 80k tokens. A standard feature or refactoring task requiring a dedicated session.
*   **L (Large):** 80k - 120k+ tokens. A complex architectural task or feature epic that will likely push the limits of the AI Studio UI's performance and the daily token quota.
...
| **Feature: Geo-location for User's Current Location** | 1000 | 2 | 90% | 0.75 | **2400** | Pro | M | **To Do** |
...
| :--- | :--- | :--- |
| **Backend: Implement `TrackedJobService` Layer** | T1 | 6.0 |

### [v0.22.0] - 2025-07-01 - Data Lifecycle Migration & Frontend Refactor
| Feature/Bugfix | Original Tier | RICE Score |
| :--- | :--- | :--- |
| **Refactor: Separate `tracked_jobs.status` into `status` and `status_reason` columns (with ENUMs)** | T1 | 4.0 |
| **Refactor: Modularize Frontend `dashboard/page.tsx`** | T1 | 300 |

### [v0.21.0] - 2025-07-01 - Backend Modularization & Frontend Stability
| Feature/Bugfix | Original Tier | RICE Score |
| :--- | :--- | :--- |
| **Refactor: Modularize Large Frontend/Backend Files (e.g., `dashboard/page.tsx`, `app.py`)** | T1 | 300 |
| **Bugfix: Status dropdown not persisting/collapsing on render size.** | T2 | 10000 |
| **Bugfix: Frontend infinite loop on dashboard load** | UI/UX Polish | N/A |
| **Bugfix: Backend deployment `ImportError`** | T1 | N/A |

### [v0.20.0] - 2025-07-01 - Architectural Definition
| Feature/Bugfix | Original Tier | RICE Score |
| :--- | :--- | :--- |
| **Backend: Define & Implement Comprehensive Data Lifecycle Management** | T1 | 2.7 |
